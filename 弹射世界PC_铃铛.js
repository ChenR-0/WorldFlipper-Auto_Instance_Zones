...
while (1) {
    ...
    if (colors.isSimilar(color, colors.parseColor("#ffffffff"))) {//有铃铛
        //点击小铃铛
        click(LD_x, LD_y);
        toast("参加救援");
        sleep(1000);
        ...
        //点击参加
        if (colors.isSimilar(images.pixel(captureScreen(), CJ_x, CJ_y), colors.parseColor("#ff29c2b5"))) {
            click(CJ_x, CJ_y);
            toast("点击参加");
            sleep(1000);
        }
        ...
        //人满失败
        if (colors.isSimilar(images.pixel(captureScreen(), FailCJ_x, FailCJ_y), colors.parseColor("#ff29c2b5"))) {
        ...
        while (1) {
            ...
            if (colors.isSimilar(WaitingFight, colors.parseColor("#ffffffff"), 8)) {
                toast("开打");
                break;
            }
            if (colors.isSimilar(Ready, colors.parseColor("#ffe7e3e7"), 8)) {
                click(ZB_x, ZB_y);
                toast("准备");
            }
            if (colors.isSimilar(isDissolution, colors.parseColor("#ff29c2b5"))) {
                click(FailCJ_x, FailCJ_y);
                toast("队伍人满/解散");
                break;
            }
        ...
        while (1) {
            ...
            if (colors.isSimilar(Finish, colors.parseColor("#ffdedbde"), 8) || (colors.isSimilar(Finish, colors.parseColor("#ffffffff")) && colors.isSimilar(Finish_Green, colors.parseColor("#ff29c2b5")))) {
                toast("结算结束");
                isSettling = 0;
                break;
            }
            ...
            else if (colors.isSimilar(Character, colors.parseColor("#ff002c4a")) || colors.isSimilar(Character, colors.parseColor("#ffffffff"), 8)) {
                click(Selltement_x, Selltement_y);
                toast("获得角色");
                sleep(3000);
                continue;
    ...
    else if (colors.isSimilar(images.pixel(img, Date_x, Date_y), colors.parseColor("#ff29c2b5"))) {//日期改变
        click(Date_x, Date_y);
        toast("日期改变");
        sleep(10000);
    }
    else if (colors.isSimilar(images.pixel(img, Login_x, Login_y), colors.parseColor("#ffded3ce"), 8)) {//检测是否到登录界面
        ...
        while (1) {//循环检测登录后点击不继续之前的铃铛
            ...
            else if(colors.isSimilar(color, colors.parseColor("#ffdedbde"), 8)){//不需要领取登录奖励 / 领取完登录奖励
                break;
            }
            else{
                //关闭公告
